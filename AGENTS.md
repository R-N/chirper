...
- **Clean, decoupled, testable** — thin controllers delegate to models; cross-cutting logic lives in traits, `Support/`, `Utils/`, and `Filters/`; behavior is exercised by backend feature tests (`tests/Feature/*`) and frontend tests (Vitest, `resources/js/**/*.spec.js`). Frontend specs default to a node environment for pure logic; component specs opt into jsdom with a `// @vitest-environment jsdom` docblock and render via `@vue/test-utils` (stub Vuetify/child components to test a component's own logic). Each piece is verifiable in isolation.
