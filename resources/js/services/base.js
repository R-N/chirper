...
class BaseService {
  ...
  checkMethod(method) {
    ...
    if (this.methods.includes(method) || !this.methods?.length) return true;
    let err = new Error(
      t("crud.method_not_allowed", {
        method: method,
  ...
  checkFiles(form, files = {}) {
    if (this.files.length == 0 || !this.files) return form;
  ...
  async call(endpoint, form = {}, method = "put", filter = true, obj = null) {
    ...
    // clear form errors
    if ("clearErrors" in form0) form0.clearErrors?.();
    if ("clearError" in form0) form0.clearError?.();

    // make api call
    try {
      res = await f(target, form, options);
      if ("reset" in form0) form0.reset?.();
    } catch (error) {
      // set form errors
      if (error?.response?.data?.errors) {
        if ("setError" in form0) form0.setError?.(error.response.data.errors);
        if ("setErrors" in form0) form0.setErrors?.(error.response.data.errors);
      }
      throw error;
    }
